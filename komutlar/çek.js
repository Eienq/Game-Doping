...
exports.run = async (client, message, emoji, args) => {
  ...
  if (!uye) return message.channel.send(acebot.setDescription("Ses odana çekilecek üyeyi belirtmelisin!")).then(x => x.delete({timeout: 5000}));
  ...
  if (message.member.hasPermission("ADMINISTRATOR")) {await uye.voice.setChannel(message.member.voice.channelID);} else {
    ...
    message.channel.send(`${uye}`, {embed: acebot.setAuthor(uye.displayName, uye.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${message.author} seni ses kanalına çekmek için izin istiyor! Onaylıyor musun?`)}).then(async msj => {
      await msj.react('✅');
